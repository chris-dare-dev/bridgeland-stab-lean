/-
Environment census of `BridgelandStabLean`, run through the interpreter.

Run: `lake env lean scratch/Census.lean`

It exists to keep `scripts/Audit.lean`'s docstring honest. That comment states
...
This is a script, NOT a gate and NOT a build target. `lean_lib "Audit"` roots at
`Audit`, so a sibling file in `scripts/` is not swept into it -- verified by
`lake build Audit` being unchanged by this file's presence. It reports the
...
module with `open BridgelandStabLean` could legally refer to it. -/
...
run_cmd do
  ...
  let ours := `BridgelandStabLean

  -- names scripts/Audit.lean actually gates
  let auditSrc ← IO.FS.readFile "scripts/Audit.lean"
  ...
  for i in [0 : mods.size] do
    let m := mods[i]!
    unless m == ours || ours.isPrefixOf m do continue
    nMods := nMods + 1
  ...
  logInfo s!"modules under BridgelandStabLean : {nMods}"
