...
`StabilityCondition/Metric/Mass/Basic.lean` and the usual mass of an arbitrary HN filtration.
...
private def factorMass (σ : StabilityCondition.WithClassMap C v) (E : C) : ℝ≥0∞ :=
  ENNReal.ofReal ‖σ.charge E‖

private lemma factorMass_eq_zero_of_isZero
    (σ : StabilityCondition.WithClassMap C v) {E : C} (hE : IsZero E) :
    factorMass σ E = 0 := by
  simp [factorMass, σ.charge_isZero hE]

private lemma HNFiltration.mass_dropFirst
    (σ : StabilityCondition.WithClassMap C v) {E : C}
    (F : HNFiltration C σ.slicing.P E) (hn : 1 < F.n)
    (hzero : IsZero (F.factor ⟨0, by lia⟩)) :
    (F.dropFirst C hn hzero).mass σ = F.mass σ := by
  unfold HNFiltration.mass
  have hhead : ENNReal.ofReal ‖σ.charge (F.factor ⟨0, by lia⟩)‖ = 0 :=
    factorMass_eq_zero_of_isZero σ hzero
  let f : Fin F.n → ℝ≥0∞ := fun i ↦ ENNReal.ofReal ‖σ.charge (F.factor i)‖
  calc
    ∑ i : Fin (F.n - 1),
        ENNReal.ofReal ‖σ.charge ((F.dropFirst C hn hzero).factor i)‖ =
...
private lemma HNFiltration.mass_appendFactor
    ...
    (G : HNFiltration C σ.slicing.P Y)
    (T : Triangle C) (hT : T ∈ distTriang C)
    (eT₁ : T.obj₁ ≅ Y) (eT₂ : T.obj₂ ≅ Z)
    (ψ : ℝ) (hψ : σ.slicing.P ψ T.obj₃)
    (hψ_lt : ∀ j : Fin G.n, ψ < G.φ j) :
    (G.appendFactor C T hT eT₁ eT₂ ψ hψ hψ_lt).mass σ =
      G.mass σ + factorMass σ T.obj₃ := by
  unfold HNFiltration.mass
  change (∑ i : Fin (G.n + 1),
      ENNReal.ofReal ‖σ.charge ((G.appendFactor C T hT eT₁ eT₂ ψ hψ hψ_lt).factor i)‖) = _
  rw [Fin.sum_univ_castSucc]
  congr 1
  · apply Finset.sum_congr rfl
    intro i _
    change ENNReal.ofReal ‖σ.charge
...
private lemma HNFiltration.mass_prefix_last
    (σ : StabilityCondition.WithClassMap C v) {E : C}
    (F : HNFiltration C σ.slicing.P E) (hn : 1 < F.n) :
    (F.prefix C (F.n - 1) (by lia) (by lia)).mass σ +
        factorMass σ (F.factor ⟨F.n - 1, by lia⟩) = F.mass σ := by
  unfold HNFiltration.mass
  let f : Fin F.n → ℝ≥0∞ := fun i ↦ ENNReal.ofReal ‖σ.charge (F.factor i)‖
  calc
    (∑ i : Fin (F.n - 1),
        ...
        factorMass σ (F.factor ⟨F.n - 1, by lia⟩) =
...
private theorem HNFiltration.exists_headTail
    ...
    ∃ (Y : C) (G : HNFiltration C σ.slicing.P Y)
      ...
      F.mass σ = factorMass σ (F.factor ⟨0, hn⟩) + G.mass σ ∧
      G.n = F.n - 1 ∧
      ∀ j : Fin G.n, ∃ i : Fin F.n,
        i.val = j.val + 1 ∧ G.φ j = F.φ i := by
  suffices hmain : ∀ (m : ℕ) {E : C} (F : HNFiltration C σ.slicing.P E),
      F.n ≤ m → ∀ hn : 0 < F.n,
      ∃ (Y : C) (G : HNFiltration C σ.slicing.P Y)
        (f : F.factor ⟨0, hn⟩ ⟶ E) (g : E ⟶ Y)
        (h : Y ⟶ (F.factor ⟨0, hn⟩)⟦(1 : ℤ)⟧),
  ...
  | succ m ih =>
      ...
      · let i0 : Fin F.n := ⟨0, hn⟩
        ...
        let G := HNFiltration.zero (P := σ.slicing.P) C (0 : C) (isZero_zero C)
        ...
        · apply isomorphic_distinguished _ (contractible_distinguished (F.factor i0))
          exact Triangle.isoMk _ _ (Iso.refl _) e.symm (Iso.refl _)
            ...
            (by simp) (by simp)
        · unfold HNFiltration.mass
          change (∑ x : Fin F.n, ENNReal.ofReal ‖σ.charge (F.factor x)‖) =
            ENNReal.ofReal ‖σ.charge (F.factor ⟨0, hn⟩)‖ + 0
          rw [add_zero]
      ...
      · have hn2 : 2 ≤ F.n := by omega
        ...
        have hGZmass : GZ.mass σ = G'.mass σ + factorMass σ T.obj₃ :=
          G'.mass_appendFactor σ oct.triangle oct.mem (Iso.refl _) (Iso.refl _)
        ...
        have hGZn : GZ.n = F.n - 1 := by
          ...
          omega
        have hphead : factorMass σ (P.factor ⟨0, hPn⟩) =
            factorMass σ (F.factor ⟨0, hn⟩) := by
          change ENNReal.ofReal ‖σ.charge (F.triangle ⟨0, by lia⟩).obj₃‖ =
            ENNReal.ofReal ‖σ.charge (F.triangle ⟨0, hn⟩).obj₃‖
          congr 4
        have hplast : factorMass σ (F.factor ilast) = factorMass σ T.obj₃ := rfl
        refine ⟨Z, GZ, f' ≫ u, v₁₃, w₁₃, h₁₃, ?_, ?_, ?_⟩
        · calc
            ...
            _ = factorMass σ (F.factor ⟨0, hn⟩) +
                (G'.mass σ + factorMass σ T.obj₃) := by
                  ...
                  rw [hplast]
                  rw [add_assoc]
            _ = factorMass σ (F.factor ⟨0, hn⟩) + GZ.mass σ := by
                  rw [hGZmass]
        · exact hGZn
        · intro j
...
theorem HNFiltration.mass_eq_zero_of_isZero
  ...
  suffices hmain : ∀ (m : ℕ) {E : C}
      (F : HNFiltration C σ.slicing.P E), F.n ≤ m → IsZero E → F.mass σ = 0 by
  ...
  | succ m ih =>
      ...
      have hhead : IsZero (F.factor ⟨0, hn⟩) :=
        F.isZero_factor_zero_of_hom_eq_zero C σ.slicing hn fun f ↦ hE.eq_of_tgt f 0
      by_cases hn1 : F.n = 1
      · unfold HNFiltration.mass
        let i0 : Fin F.n := ⟨0, hn⟩
        calc
          (∑ i : Fin F.n, ENNReal.ofReal ‖σ.charge (F.factor i)‖) =
              ENNReal.ofReal ‖σ.charge (F.factor i0)‖ := by
            apply Finset.sum_eq_single i0
            · intro j _ hji
              exact absurd (Fin.ext (by have := j.isLt; omega)) hji
            · simp
          _ = 0 := factorMass_eq_zero_of_isZero σ hhead
      · have hn2 : 1 < F.n := by omega
        let G := F.dropFirst C hn2 hhead
        rw [← F.mass_dropFirst σ hn2 hhead]
        exact ih G (by change F.n - 1 ≤ m; omega) hE

/-- **HN mass is independent of the chosen HN filtration.** -/
theorem HNFiltration.mass_eq_mass
    (σ : StabilityCondition.WithClassMap C v) {E : C}
    (F G : HNFiltration C σ.slicing.P E) : F.mass σ = G.mass σ := by
  suffices hmain : ∀ (m : ℕ) {E : C}
      (F G : HNFiltration C σ.slicing.P E), F.n + G.n ≤ m →
      F.mass σ = G.mass σ by
  ...
  | zero =>
      ...
      rw [F.mass_eq_zero_of_isZero σ hE, G.mass_eq_zero_of_isZero σ hE]
  | succ m ih =>
      intro E F G hlen
      ...
      · have hnF2 : 1 < F.n := by
        ...
        rw [← F.mass_dropFirst σ hnF2 hzF]
      ...
      obtain ⟨YG, TG, fG, gG, dG, htriG, hmassG, hnTG, hφTG⟩ :=
        G.exists_headTail σ hnG
      let φ := F.φ ⟨0, hnF⟩
      have hheadF : σ.slicing.geProp C φ (F.factor ⟨0, hnF⟩) := by
        apply σ.slicing.geProp_of_hn C
      ...
      have htailMass : TF.mass σ = TG.mass σ := by
        ...
        simpa using hrec
      calc
        F.mass σ = factorMass σ (F.factor ⟨0, hnF⟩) + TF.mass σ := hmassF
        _ = factorMass σ (G.factor ⟨0, hnG⟩) + TG.mass σ := by
...
theorem stabilityMass_eq_mass
  ...
  · refine iSup_le fun G ↦ ?_
    exact le_of_eq (G.mass_eq_mass σ F)
  · exact le_iSup (fun G : HNFiltration C σ.slicing.P E ↦ G.mass σ) F

/-- The ordinary HN mass is always finite. -/
theorem stabilityMass_ne_top
    (σ : StabilityCondition.WithClassMap C v) (E : C) :
    stabilityMass σ E ≠ ⊤ := by
...
theorem stabilityMass_lt_top
    (σ : StabilityCondition.WithClassMap C v) (E : C) :
    stabilityMass σ E < ⊤ :=
  (lt_top_iff_ne_top).2 (stabilityMass_ne_top σ E)
...
theorem stabilityMass_toReal_eq_sum
    ...
    (stabilityMass σ E).toReal =
      ∑ i : Fin F.n, ‖σ.charge (F.factor i)‖ := by
  rw [stabilityMass_eq_mass σ F]
  unfold HNFiltration.mass
...
theorem HNFiltration.exists_headTail_mass
    ...
    ∃ (Y : C) (G : HNFiltration C σ.slicing.P Y)
      ...
      ∀ j : Fin G.n, ∃ i : Fin F.n,
        i.val = j.val + 1 ∧ G.φ j = F.φ i := by
  obtain ⟨Y, G, f, g, h, hT, hmass, hGn, hφ⟩ := F.exists_headTail σ hn
  refine ⟨Y, G, f, g, h, hT, ?_, hGn, hφ⟩
  have hsplit : stabilityMass σ E =
      stabilityMass σ (F.factor ⟨0, hn⟩) + stabilityMass σ Y := by
    rw [stabilityMass_eq_mass σ F, hmass, stabilityMass_eq_mass σ G,
      stabilityMass_eq_ofReal_norm_charge σ (F.semistable ⟨0, hn⟩)]
    rfl
  rw [hsplit, ENNReal.toReal_add]
  · simp [stabilityMass_eq_ofReal_norm_charge σ (F.semistable ⟨0, hn⟩)]
  · exact stabilityMass_ne_top σ _
  · exact stabilityMass_ne_top σ _
