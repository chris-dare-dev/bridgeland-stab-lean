...
namespace BridgelandStabLean.TStructure
...
theorem exists_shiftedTriangleLTGE_iso :
  ...
  t.triangle_iso_exists (shiftedTriangleLTGE_distinguished t a n X)
    ...
    (by dsimp; infer_instance) (by dsimp; infer_instance) (by lia)
...
theorem shiftedTriangleLTGEIso_hom₂ :
    (shiftedTriangleLTGEIso t a n X).hom.hom₂ = 𝟙 _ :=
...
theorem truncLTShiftIso_hom_comp_truncLTι :
  ...
  simpa [truncLTShiftIso, shiftedTriangleLTGE, shiftedTriangleLTGEIso_hom₂]
    using ((shiftedTriangleLTGEIso t a n X).hom.comm₁).symm
