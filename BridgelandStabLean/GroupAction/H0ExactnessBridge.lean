...
Here “unconditional” means that the main theorems do not assume an existing
`Functor.IsHomological` instance.  They still take `h : HeartStabilityData C`:
the proofs use only its t-structure component `h.t`; the bundle's boundedness,
stability function, and Harder--Narasimhan data are not used.  The result is
`ShortComplex.Exact` at the middle object, not `ShortExact`; in particular, it
does not assert that the first map is monic.

The auxiliary `H⁰'` functor is a proof-friendly normal form.  Its
homologicality is transported across the canonical natural isomorphism to the
primary heart-valued `H⁰` functor.  Every instance introduced below is local
to a proof: this module installs no global homologicality instance.  It also
proves no global mass theorem.
-/
...
/-- Converting a map `X ⟶ E` to the `H⁰'` normal form and evaluating it back
at a nonpositive object recovers the original map. -/
@[simp]
...
/-- Evaluating a map out of `H⁰'(X)` and reconstructing it through the
nonpositive universal property recovers the original heart morphism. -/
@[simp]
...
/-- Evaluation from the `H⁰'` normal form sends the zero heart morphism to the
zero morphism in the triangulated category. -/
@[simp]
...
/-- The map into the `H⁰'` normal form is natural under precomposition between
nonpositive objects. -/
@[reassoc]
...
/-- Evaluation from the `H⁰'` normal form is natural under precomposition
between nonpositive objects. -/
@[reassoc]
...
in the heart is exact at its middle object, without assuming an external
homological-functor instance.  This is `ShortComplex.Exact`, not
`ShortExact`. -/
...
/-- The canonical `H⁰'` functor of the t-structure component `h.t` is
homological.  This construction is unconditional only in the sense that it
requires no prior `Functor.IsHomological` instance; it still takes
`h : HeartStabilityData C`, while using only `h.t` and not the bundled
boundedness, stability function, or Harder--Narasimhan data.  The proof truncates
the source of an arbitrary triangle at degree zero, uses exactness for the
nonpositive-source triangle, and compares it to the original complex through
a morphism that is an isomorphism on the first two terms and monic on the
third.  The theorem returns a value and installs no global instance. -/
...
/-- Unconditional homologicality, in the no-prior-instance sense above,
transported from the proof-friendly `H⁰'` normal form across the canonical
natural isomorphism to the primary heart-valued `H⁰` functor.  This theorem
returns a value and installs no global instance. -/
