...
/-!
# The `Aut` action on stability conditions, as a genuine `MulAction`

`AutStabilityAction.lean` produced `actStabAut`: a well-defined *map* on
...
def AutPairQuot (v : K₀ C →+ Λ) := Quotient (AutPair.setoid v)
...
`AutQuot.mk`: `AutPairQuot` is a plain `def`, so a bare `Quotient.mk` leaves
`•` unable to find its instance. -/
def mk (a : AutPair v) : AutPairQuot v := Quotient.mk _ a

/-- **The `Aut` half of §8, as a group action.**

This is the statement `AutStabilityAction.lean` could not make. -/
noncomputable instance mulAction :
    MulAction (AutPairQuot v) (StabilityCondition.WithClassMap C v) where
  smul q σ := Quotient.liftOn q (fun a => a.act σ) (fun _ _ h => AutPair.act_congr h σ)
  one_smul σ := AutPair.act_id σ
  mul_smul q r σ := by
    induction q using Quotient.inductionOn with | _ a =>
    induction r using Quotient.inductionOn with | _ b =>
