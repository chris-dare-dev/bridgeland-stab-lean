...
/-!
# Topology and simple connectedness of `G̃L⁺(2, ℝ)`

This file begins the topological half of the universal-cover theorem.  The
group-theoretic half is in `GLTildeFibre.lean` and `GLTildeSurj.lean`.
...
`GLTildeCover.lean` continues from these coordinates, gives global circle
coordinates on `GL⁺(2, ℝ)`, and proves that the matrix projection is a
covering map. `GLTildeTopologicalGroup.lean` then proves that multiplication
...
theorem is proved separately in `GLTildeCover.lean`. -/
