...
/-!
# The action on prestability conditions

Lane-1 step 3b. Both factors of `G̃L⁺(2, ℝ)` finally do their jobs at once:

```
(x • σ).slicing = x.shift • σ.slicing        -- phases relabelled  (3a)
(x • σ).Z       = actC x.mat ∘ σ.Z           -- charge transformed (here)
```
...
That is the entire content of this file, and it is why step 2 defined
`Compatible` the way it did.
...
`notes/anchor-api-map.md` §4. That is step 3c.

Note this file does **not** need `[IsTriangulated C]`: the anchor declares it
at file scope in `StabilityCondition/Defs.lean` but
`PreStabilityCondition.WithClassMap` does not depend on it. Only 3c will.
-/
