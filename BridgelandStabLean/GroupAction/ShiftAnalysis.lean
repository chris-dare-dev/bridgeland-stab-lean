...
/-!
# Normalized shifts are uniformly continuous

Groundwork for lane-1 step 3c, and the only genuinely new *analysis* in this
repo.

The anchor's `Slicing.IsLocallyFinite` quantifies **one** radius `η` over
...
`exists_radius` is the form step 3c consumes.
-/
...
/-- **The form step 3c consumes.** Windows can be shrunk uniformly: given a
target width `w` and a ceiling `M`, there is a radius `η < M` such that `f`
