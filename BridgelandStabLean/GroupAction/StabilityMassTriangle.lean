...
/-!
# The HN mass triangle inequality and topology compatibility

`StabilityMassTriangleInequality` — `m(E) ≤ m(A) + m(C)` for every distinguished
triangle `A → E → C` — is the categorical mass input to Proposition 8.1's
second clause. This file retains the PR #70 reduction from the general
statement to the special case in which the left-hand object is *semistable*:

```
StabilityMassSemistableTriangleInequality → StabilityMassTriangleInequality
```

It then proves that semistable-left case through the polygonal and heart-level
mass estimates, obtaining unconditional theorem values for the general mass
triangle inequality and compatibility of `stabilityDist` with the existing
Section 6 topology. No global topology or metric instance is installed here.

## The retained categorical reducer

The reduction is a theorem, not a definition. It isolates the strict
intermediate obligation: `StabilityMassSemistableTriangleInequality` quantifies over
...
`StabilityMassTriangleInequality` and keep doing so. The proof chain below now
inhabits the intermediate proposition and feeds the result back through this
reducer.
...
`StabilityMassTriangleInequality`, and it is the intermediate input discharged
below: `stabilityMassTriangleInequality_of_semistable` derives the general
statement from it. Like the general one, this is a `def … : Prop` and
...
On its own this reducer moves the obligation onto
`StabilityMassSemistableTriangleInequality`; the proof chain below supplies
that theorem value. -/
...
`K₀(C)`. This lets the ordinary heart-equivalence API be reused for a condition
defined with an arbitrary class map. -/
...
/-- The observable stability condition retains the original slicing. -/
@[simp]
...
/-- The observable stability condition retains the original object charge. -/
@[simp]
...
/-- The observable heart stability function evaluates to the original object charge. -/
@[simp]
...
theorem stabilityMassSemistableTriangleInequality :
    StabilityMassSemistableTriangleInequality (C := C) (v := v) := by
  intro σ φ T hT h₁
  let θ : ℝ := 1 - φ
...
/-- HN mass is subadditive across every distinguished triangle. -/
theorem stabilityMassTriangleInequality :
    StabilityMassTriangleInequality (C := C) (v := v) :=
  stabilityMassTriangleInequality_of_semistable
    stabilityMassSemistableTriangleInequality

/-- Full-distance balls form a basis for the existing Section 6 topology. -/
