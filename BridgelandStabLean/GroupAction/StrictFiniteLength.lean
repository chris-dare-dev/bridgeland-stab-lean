...
/-!
# Strict finite length transfers along a functor, from a strict hypothesis

The reusable centrepiece of the third piece the `Aut` action on stability
conditions needs.

The anchor has a general transfer lemma,
...
This file extracts that argument once, for an arbitrary functor. The anchor's
...
composition is not available in general** — the anchor proves it only for
...
anchor's `interval_thinFiniteLength_of_inclusion_strict` cannot be reused for
...
include hF harr in
/-- **Strict-Artinian transfers, from a strict hypothesis.** The anchor's
...
Contrast `relabel_intervalProp`, where a phase relabelling moved the endpoints
and forced the uniform-continuity argument of step 3c. -/
