...
The anchor needs this repeatedly and never states it — `Deformation/DeformedGtLe.lean`
...
scope here. -/
@[cites "stmt:a520a8d4f877:bridgeland2007.lem-8.2" (relation := no_claim)
        (note := "Same non-implication as mapEquiv_slicingDist: the distance is the anchor's slicingDist, not Bridgeland's d, and d omits nothing while slicingDist omits the mass ratio. What this adds over that theorem is only the carrier -- the statement is now about stability conditions rather than bare slicings, matching the paper's Stab(D). It is still not the paper's isometry claim.")]
...
`Quotient.inductionOn` is safe here without a well-definedness side goal: the
...
theorem AutPairQuot_smul_slicingDist (g : BridgelandStabLean.GroupAction.AutPairQuot v)
    (σ τ : StabilityCondition.WithClassMap C v) :
