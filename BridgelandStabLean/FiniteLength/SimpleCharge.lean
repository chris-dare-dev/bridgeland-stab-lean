...
* **The anchor does not have it either.** `GrothendieckGroup/` builds `K₀` as
  a quotient of a free abelian group by a relation subgroup, and nothing there
  or anywhere else in the anchor says that quotient is free on the simples.
...
anchor's, in `ForMathlib/Analysis/SpecialFunctions/Complex/ArgConvexity.lean`.
What the anchor does **not** have, and what is added here, is closure under
...
dot-notation extensions on anchor types, each of which is a name a future
anchor bump could collide with (CLAUDE.md §1); there is no reason to make that
...
/-! ### Two closure properties the anchor does not carry -/
